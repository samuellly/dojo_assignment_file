...
class UserManager(models.Manager):
    def register(self, input):
        ...
        if not input['hire_date']:
            errors.append('Please enter the birthday')
        else:
            date = datetime.strptime(input['hire_date'], "%Y-%m-%d")
            if datetime.today() <= date:
                errors.append('birthday cannot be after today')

        if len(errors) == 0:
            pwHash = bcrypt.hashpw(input['password'].encode(), bcrypt.gensalt().encode())
            user = User.objects.create(name=input['name'], username=input['username'], hire_date=input['hire_date'], password=pwHash)
            return (True, user)
